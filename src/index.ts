...
export async function createVerificationCode({ verificationCode, email }: { verificationCode: string, email: string }) {

    try {
        const newVerificationCode = await VerificationCode.create({ verificationCode: verificationCode, email: email, createdAt: new Date() })
...
export async function verifyEmail({ email, codeThatTheUserHas }: { email: string, codeThatTheUserHas: string }) {
    ...
    try {
        ...
        if (findCode.verificationCode === codeThatTheUserHas) {
            ...
            const user = await User.findOneAndUpdate({ email: email }, { emailVerified: new Date() }, { new: true })
...
export async function updateName({ name, lastname, email }: { name: string, lastname: string, email: string }) {
    try {
        const user = await User.findOneAndUpdate({ email: email }, { name: name, lastname: lastname }, { new: true })
        return user;
    } catch (e) {
        console.log(e);
